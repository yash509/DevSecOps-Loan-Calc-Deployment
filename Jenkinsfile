pipeline {
    ...
    stages {
        ...
        stage("Sonarqube Analysis ") {                         
            steps {
                //dir('Band Website') {
                    ...
                    sh '''$SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=LC-application \
                    -Dsonar.projectKey=LC-application'''
                    //}
                }
            }
        }
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag loan-calc yash5090/loan-calc:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/loan-calc:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/loan-calc:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/loan-calc:latest'
                       sh 'docker-scout cves yash5090/loan-calc:latest'
                       sh 'docker-scout recommendations yash5090/loan-calc:latest'
                       sh 'docker-scout attestation yash5090/loan-calc:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/loan-calc:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop loan-calc | true"
                sh "docker rm loan-calc | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name loan-calc -p 5000:80 yash5090/loan-calc:latest' 
                //}
            }
        }
